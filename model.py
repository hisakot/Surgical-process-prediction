...
class ResNet50LSTM(nn.Module):
    def __init__(self, pretrained, num_input_channel, num_output):
        ...
        self.resnet50 = models.resnet50(pretrained=True, num_classes=1000)
        self.resnet50.conv1 = nn.Conv2d(num_input_channel, 64, kernel_size=7,
			                stride=2, padding=3, bias=False)
        self.resnet50.fc = nn.Linear(self.resnet50.fc.in_features, 100)

        # TODO
        # nn.LSTM(input_size, hidden_size, sequence_length)
        self.lstm = nn.LSTM(input_size=100, hidden_size=200, batch_first=True,
			    num_layers=1, bidirectional=False, dropout=0.3)
        self.fc = nn.Linear(200, num_output)
	...
	lstm_x = list()
        for t in range(x.size(1)):
                with torch.no_grad():
                    x = self.resnet50(x[:, t, :, :, :].float())
                    lstm_x.append(x.unsqueeze(1))
	lstm_x = torch.cat(lstm_x, axis=1)
        out, (h_n, c_n) = self.lstm(lstm_x, None) # x = (batch_size, 1, 300)
        out = out[:, -1, :]
        out = self.fc(out)
        return out
